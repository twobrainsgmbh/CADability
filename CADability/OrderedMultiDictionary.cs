...
namespace Wintellect.PowerCollections
{
    ...
    public class OrderedMultiDictionary<TKey, TValue> : MultiDictionaryBase<TKey, TValue>, ICloneable
    {
        ...
        internal int TotalCount // wir brauchen das halt
        {
            get
            {
        ...
        internal IEnumerable<KeyValuePair<TKey, TValue>> AllItems
        {
            get
            {
                return tree;
        ...
        internal new KeyValuePair<TKey, TValue>[] Debug
        {
            get
            {
                List<KeyValuePair<TKey, TValue>> res = new List<KeyValuePair<TKey, TValue>>(tree);
                return res.ToArray();
            }
        }
        internal TKey[] DebugKeys
        {
            get
            {
                List<KeyValuePair<TKey, TValue>> res = new List<KeyValuePair<TKey, TValue>>(tree);
                TKey[] resk = new TKey[res.Count];
        ...
        internal TValue[] DebugValues
        {
            get
            {
                List<KeyValuePair<TKey, TValue>> res = new List<KeyValuePair<TKey, TValue>>(tree);
                TValue[] resv = new TValue[res.Count];
                for (int i = 0; i < res.Count; ++i)
                {
                    resv[i] = res[i].Value;
                }
        ...
        internal KeyValuePair<TKey, TValue> FirstItem
        {
            get
            {
                return tree.GetItemByIndex(0);
            }
        }
        internal KeyValuePair<TKey, TValue> LastItem
        {
            get
            {
                return tree.GetItemByIndex(tree.ElementCount - 1);
            }
        }
        internal List<TValue> SortedValues
        {
            get
            {
                List<KeyValuePair<TKey, TValue>> res = new List<KeyValuePair<TKey, TValue>>(tree);
                List<TValue> resv = new List<TValue>(res.Count);
                for (int i = 0; i < res.Count; ++i)
                {
                    resv.Add(res[i].Value);
                }
                return resv;
            }
        }
        ...
        public TValue Item(TKey key)
        {
            ...
            if (tree.FirstItemInRange(KeyRange(key), out item) >= 0)
            {
                return item.Value;
            }
            else
