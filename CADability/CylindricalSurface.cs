...
namespace CADability.GeoObject
{
		...
		IExportStep, ISurfaceOfArcExtrusion, ICylinder, IJsonSerialize
	{
		...
		public override IPropertyEntry GetPropertyEntry(IFrame frame)
		{
			...
			GeoPointProperty location = new GeoPointProperty(frame, "CylindricalSurface.Location");
			location.ReadOnly = true;
			location.OnGetValue = () => toCylinder * GeoPoint.Origin;
			se.Add(location);
			GeoVectorProperty dirX = new GeoVectorProperty(frame, "CylindricalSurface.DirectionX");
			dirX.ReadOnly = true;
			dirX.IsAngle = false;
			dirX.OnGetValue = () => toCylinder* GeoVector.XAxis;
			se.Add(dirX);
			GeoVectorProperty dirY = new GeoVectorProperty(frame, "CylindricalSurface.DirectionY");
			dirY.ReadOnly = true;
			dirY.IsAngle = false;
			dirY.OnGetValue = () => toCylinder * GeoVector.YAxis;
			se.Add(dirY);
			if (Precision.IsEqual(RadiusX, RadiusY))
			{
				...
				radius.SetDouble(RadiusX);
				radius.OnGetValue = () => RadiusX;
				se.Add(radius);
			}
		...
		/// </summary>
		public bool IsRealCylinder => Math.Abs(XAxis.Length - YAxis.Length) < Precision.eps;
